...
$$
\tr(\boldsymbol \Gamma) = (n-p-f)\left(1-2\theta_0+\sum_{i=-p}^{+f} \theta_i^2\right).
$$
En revanche il est difficile d'avoir une formule simplifiée pour le dénominateur $\tr(\boldsymbol \Gamma^2).$
Ce dernier peut être calculé en reconstruisant la matrice $\boldsymbol \Gamma$ (produit de matrices de taille $n\times n$) ou par le produit d'une matrice de taille $1\times (p+f+1)$ et d'une matrice de taille $(p+f+1)\times(p+f+1)$ pour réduire le temps de calcul (voir @sec-df-var) :
$$
\tr((\boldsymbol \Gamma\transp{\boldsymbol \Gamma})^2)=(n-(p+f))L_0^2+2\sum_{k=1}^{p+f}(n-(p+f)-k)L_k^2
$$
...
Le calcul de $\nu$ peut aussi être approximé par $\tr(\boldsymbol \Gamma)$ (`rjd3filters::confint_filter(exact_df = FALSE)`) ce qui permet de réduire davantage le temps de calcul^[
...
Il est divisé d'environ 350 en utilisant la formule proposée dans cet article (environ 0,31 milliseconde) et d'environ 5 800 en utilisant l'approximation $\nu\simeq \tr(\boldsymbol \Gamma)$ (environ 0,02 milliseconde).
]. 

# Résultats

Les différentes méthodes sont tout d'abord comparées sur des séries simulées (choc de 10 %, tendance de degré 0, 1 ou 2) puis sur des séries réelles
...
```{r}
res <- readRDS(file.path("..", "results", "simul", "simul_ao_td2.RDS"))
graph_est <- get_all_plots(res, y_as_plot = y_as_plot, add_y = add_y)
graph_est$CLF <- NULL
graph_y <- plot_y(res)
graph_ci <- plot_confint(
...
#### Tendance de degré 2
```{r}
res <- readRDS(file.path("..", "results", "simul", "simul_ls_td2.RDS"))
graph_est <- get_all_plots(res, y_as_plot = y_as_plot, add_y = add_y)
graph_est$CLF <- NULL
graph_y <- plot_y(res)
graph_ci <- plot_confint(
...
Soient $\boldsymbol\theta = \begin{pmatrix}\theta_{-p},\dots,\theta_f\end{pmatrix}$ et $\boldsymbol \Gamma = \transp{(\boldsymbol I - \boldsymbol H)}(\boldsymbol I - \boldsymbol H\boldsymbol)\$ avec :
$$
\boldsymbol H=\begin{pmatrix}
&&\boldsymbol0_{p\times n} \\
\theta_{-p} & \cdots & \theta_f  & 0 & \cdots\\
...
On a ${\boldsymbol \Gamma}_{i,j}=w_{i-j}\1_{i\in[p,n-f]}$ et :
\begin{align*}
({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})_{i,j} &=\sum_{k=1}^n{\boldsymbol \Gamma}_{i,k}{\boldsymbol \Gamma}_{j,k}=\sum_{k=1}^nw_{i-k}w_{j-k} \1_{(i,j)\in[p,n-f]^2}\\
...
Puis :
\begin{align*}
(({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})^2)_{i,i} &= \sum_{j=1}^n({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})_{i,j}({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})_{j,i}\\
&=\sum_{j=1}^n({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})_{i,j}^2\text{ car la matrice }{\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}}\text{ est symétrique} \\
...
\end{align*}
Donc $(({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})^2)_{i,i} = 0$ si $i\notin \in[p,n-f].$
Pour $i\in[p,n-f],$ comme pour tout $k\in [-p,f]$ on a $w_{i-j-k} =0$ si $j<i-(f+p)$ ou si $j>i-(f+p)$, il vient :
\begin{align*}
(({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})^2)_{i,i} &= \sum_{j=\max(i-(f+p),1)}^{\min(i+(f+p),n)}\left(\sum_{k=-p}^fw_{k}w_{i-j-k}\right)^2 \\
...
En somme :
\begin{align*}
\tr(({\boldsymbol \Gamma}\transp{{\boldsymbol \Gamma}})^2)& = \sum_{i=p}^{n-f} \sum_{j=-(f+p)}^{f+p}\left(L_{|j|}\right)^2\1_{j\in [1-i,n-i]} \\
