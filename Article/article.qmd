...
Une moyenne mobile est une méthode statistique qui consiste à appliquer une moyenne pondérée glissante à une série chronologique : à chaque date $t$ on calcule une moyenne pondérée de $p$ points passés et $f$ points futurs où $p,f\geq0$ dépend de la moyenne mobile.
]. 
...
alors que les méthodes de désaisonnalisation (comme la méthode X-13ARIMA) ont un module de correction des points atypiques.


Par, les moyennes mobiles étant des opérateurs linéaires, ils sont sensibles à la présence de points atypiques.
...
Si l'on souhaite que le choc soit affecté à la tendance-cycle et non pas à l'irrégulier, il faut utiliser le régresseur $O_{t+j}=O_{t+j}^{AO}=\1_{t+j=t_0}$ lorsque $t>t_0$ et $O_{t+j}=-O_{t+j}^{AO}=\1_{t+j\ne t_0}$ lorsque $t\leq 0$
...
Ce dernier peut être calculé en reconstruisant la matrice $\boldsymbol \Delta$ (produit de matrices de taille $n\times n$) ou par le produit d'une matrice de taille $1\times (p+f+1)$ et d'une matrice de taille $(p+f+1)\times(p+f+1)$ pour réduire le temps de calcul (voir annexe [-@sec-df-var]) :
$$
...
Pour les estimations en temps réel, les tendances sont surestimées mais les révisions faibles : cela s'explique par le faible degré de courbure de la série simulée.
...
Dans cette section, nous nous concentrerons sur l'illustration d'autres cas : un choc ponctuel suivi d'un choc permanent (@sec-ex-ao-ls), la crise financière de 2008 (@sec-ex-crise-financiere), la crise du COVID-19 (@sec-ex-covid}) et un autre point de retournement qui n'est pas associé à un choc (@sec-ex-retournement).


### Choc ponctuel (AO) suivi d'un choc permanent (LS) {#sec-ex-ao-ls}

L'entrée en vigueur de la procédure d'essai mondiale harmonisée pour les véhicules légers (*Worldwide harmonized Light vehicles Test Procedures*, WLTP) en septembre 2018 a eu pour effet une forte hausse des immatriculations de voitures particulières neuves^[
Série étudiée corrigée des variations saisonnières et des jours ouvrables, téléchargée fin octobre 2024 à l'adresse : <https://www.insee.fr/fr/statistiques/serie/010756763>. 
] en août 2018 (choc ponctuel, @fig-imat2018-y), les constructeurs ayant déstocké des modèles qui n'étaient plus autorisés à la vente à partir de cette date.
À partir du mois de septembre, le niveau des immatriculations était plus faible par contre-coup de la hausse du mois d'août (choc permanent).
...
La crise financière de 2008 est illustrée à partir de données issues de la base FRED-MD [@fredmd] contenant des séries économiques sur les États-Unis^[
Les séries étudiées correspondent à la base publiée en novembre 2022.
] : les ventes au détail et services de restauration (série `RETAILx`, utilisée en logarithme).
...
Le COVID-19 est illustré sur l'indice de la production industrielle (IPI) dans l'industrie manufacturière, corrigé des variations saisonnières (CVS) et des jours ouvrables (CJO), publié par l'Insee^[
<https://www.insee.fr/fr/statistiques/serie/010768307>.
...
### Point de retournement {#sec-ex-retournement}

Les points de retournement sont illustrées à partir de données issues de la base FRED-MD [@fredmd] contenant des séries économiques sur les États-Unis^[
Les séries étudiées correspondent à la base publiée en novembre 2022.
] : le niveau d'emploi aux États-Unis (série `CE16OV`, utilisée en logarithme) et les ventes au détail et services de restauration (série `RETAILx`, utilisée en logarithme)
...
#| fig-height: 2
ggdemetra3::ggplot_coef(CLF, q= 0:6) + ggdemetra3::ggplot_coef(CLF_CN, q= 0:6)
```
...
Trois chocs ponctuels s'observent clairement sur les mois d'août de 1998, 1999 et 2004.
...
#| fig.cap: "IPI CVS-CJO dans la construction de véhicules automobiles autour du point atypique d'août 1998"
#| fig-height: 2
graph_y[[1]]
```
...
#| fig.cap: "IPI CVS-CJO dans la construction de véhicules automobiles autour du point atypique d'août 1999"
#| fig-height: 2
graph_y[[2]]
```
...
#| fig.cap: "IPI CVS-CJO dans la construction de véhicules automobiles autour du point atypique d'août 2004"
#| fig-height: 2
graph_y
```
...
Pour l'emploi le retour à la normal est progressif à partir d'avril 2020 (@fig-ce16ov-covid-y) alors qu'il est rapide pour les ventes au détail et services de restauration (@fig-retailx-covid-y).

Comme pour l'IPI dans l'industrie manufacturière, les méthodes robustes effacent complètement le choc (figures [-@fig-ce16ov-covid-est] et [-@fig-retailx-covid-est]), ce qui ne permet pas d'analyser les points de retournement.
Les moyennes mobiles linéaires classiques (Henderson/Musgrave et CLF) détectent les points de retournements avec un décalage : pic en octobre 2019 et creux en avril 2020 pour les ventes au détail et services de restauration et juin 2020 pour l'emploi.
...
```{r}
#| label: fig-ce16ov-covid-y
#| fig.cap: "Emploi (en logarithme) aux États-Unis autour de mars 2020"
#| fig-height: 2
...
```{r}
#| label: fig-retailx-covid-y
#| fig.cap: "Ventes au détail et services de restauration (en logarithme) aux États-Unis autour de mars 2020"
#| fig-height: 2
