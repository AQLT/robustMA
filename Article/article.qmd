...
plot_specific_model <- function(num_mod) {
	...
	(get_all_plots(
		readRDS(file.path("..", "results", "AOLS", "immat2018.RDS")), modeles_retenus = num_mod
	)[[1]] + 
		ggplot2::ggtitle("Immatriculations de voitures\nparticulières neuves")) +
	(get_all_plots(
		readRDS(file.path("..", "results", "LSLS", "retailx2008.RDS")), modeles_retenus = num_mod
	)[[1]] + 
		ggplot2::ggtitle("Ventes au détail et\nservices de restauration\n(États-Unis)")) +
	(get_all_plots(
		readRDS(file.path("..", "results", "LSLS", "ipi_manufcovid.RDS")), modeles_retenus = num_mod
	)[[1]] + 
		ggplot2::ggtitle("IPI industrie manuf.")) +
	(get_all_plots(
		readRDS(file.path("..", "results", "AO", "ce16ov2001.rds")), modeles_retenus = num_mod
	)[[1]] + 
		ggplot2::ggtitle("Emploi (États-Unis)"))
}
models <- c(
	1, 2, # Pas CLF et ALF
	4, 5, 6, 7, 8, 9, 10)
...
En conclusion, cette étude montre comment, à partir des moyennes mobiles de Henderson et de Musgrave (répanduent pour l'extraction de la tendance-cycle) il est possible d'ajouter des régresseurs externe pour construire des moyennes mobiles avec une meilleure décomposition tendance-cycle et irrégulier.
...
Dans cette étude, les performances des moyennes mobiles linéaires sont comparées à un ensemble de méthodes non-linéaires robustes modélisant une tendance localement linéaire (comme le filtre de Musgrave mais contrairement au filtre de Henderson qui modélisent une tendance localement cubique).
Ces méthodes sont pour la plupart basées sur la médiane et conduisent parfois à des révisions fortes lors de l'estimation en temps-réel, ce qui suggère, comme notamment indiqué par @gather2006online, d'utiliser des quantiles plus élevés que la médiane pour les estimations en temps réel avec les méthodes robustes.
...
```{r}
#| label: fig-lmsd2
#| fig-cap: "Estimations en temps réel de la tendance-cycle pour la méthode des moindres carrés médians (LMS) en modélisation une tendance locale de degré 2"
#| fig-height: 6
...
```{r}
#| label: fig-ltsd2
#| fig-cap: "Estimations en temps réel de la tendance-cycle pour la méthode des moindres carrés élagués (LMS) en modélisation une tendance locale de degré 2"
#| fig-height: 6
p_lts
```
