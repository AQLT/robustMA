...
::: {.remark}
La matrice $\boldsymbol H$ pourrait également être construite en prenant en complétant les $p$ premières et $f$ dernières lignes par les moyennes mobiles asymétriques qui sont utilisées.
Toutefois, puisque les modèles sous-jacent aux moyennes mobiles asymétriques sont différents de ceux des moyennes mobiles symétriques (par exemple Musgrave modélise une tendance de degré 1 alors que Henderson modélise une tendance de degré 3), on peut supposer les variances différentes pour chaque moyenne mobile et estimer celles associées aux moyennes mobiles asymétriques en utilisant l'ensemble des données.
C'est l'approche utilisée dans `rjd3filters` : l'intervalle de confiance sera donc plus grand pour les estimations intermédiaires que pour les estimations finales (ce à quoi l'on pourrait s'attendre !).
:::
...
graph_ci <- plot_confint(
	...
	robust_f = list(
		"MM robuste LS-LS" = robust_ff[["lsls"]],
		"MM robuste AO-AO\n(sur TC)" = robust_ff[["aoaoinv"]],
		"MM robuste AO-LS\n(sur TC)" = robust_ff[["aoinvls"]]
		),
	show.legend = TRUE
)
# graph_y
