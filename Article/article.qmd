...
```{r}
res <- readRDS(file.path("..", "results", "simul", "simul_ao.RDS"))
...
```{r}
#| label: fig-simul_ao-out1-y
#| fig.cap: "Série brute"
graph_y
```

```{r}
#| label: fig-simul_ao-out1-est
...
```{r}
res <- readRDS(file.path("..", "results", "simul", "simul_ls.RDS"))
graph_est <- get_all_plots(res, y_as_plot = y_as_plot, add_y = add_y)
graph_est$CLF <- NULL
graph_y <- plot_y(res)
graph_ci <- plot_confint(
...
```{r}
#| label: fig-simul_ls-out1-y
#| fig.cap: "Série brute"
graph_y
```

```{r}
#| label: fig-simul_ls-out1-est
#| fig.cap: "Estimations en temps réel de la tendance-cycle"
#| fig-height: 8
#| 
patchwork::wrap_plots(graph_est, ncol = 3) 
