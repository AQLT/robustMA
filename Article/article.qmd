...
Ce dernier peut être calculé en reconstruisant la matrice $\boldsymbol \Gamma$ (produit de matrices de taille $n\times n$) ou par le produit d'une matrice de taille $1\times (p+f+1)$ et d'une matrice de taille $(p+f+1)\times(p+f+1)$ pour réduire le temps de calcul (c'est ce qui est fait par défaut dans `rjd3filters::confint_filter()`, voir @sec-df-var).
...
$$
et $\boldsymbol I_{n-p-f}$ la matrice identité de taille $n-p-f.$
L'objectif de cette annexe est de simplifier le calcul de $\tr(\boldsymbol \Gamma^2)$
On a donc :
$$
\hat\sigma^2=\frac{1}{\tr{\boldsymbol \Gamma}}\transp{\boldsymbol Y}\boldsymbol \Gamma\boldsymbol Y
$$
