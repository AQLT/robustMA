...
Le choc ponctuel s'observe en août 2018, suivi d'un choc permament en septembre 2018.
...
Les méthodes robustes ne sont pas affectées par les chocs de cette période (@fig-ipi-manuf-covid-est) : ces méthodes ne permettent donc pas de reproduire la spécificité économique de cette période. 
Certaines d'entre elles conduisent également à de fortes révisions.
Les moyennes mobiles de Musgrave et CLF conduisent, encore une fois, à détecter les points de retournement avec un décalage.
...
La @fig-ipi-manuf-covid-ci compare les estimations des moyennes mobiles robustes construites en modélisant deux chocs ponctuels consécutifs (dont on attribuerait l'effet à la tendance-cycle plutôt qu'à l'irrégulier) et deux chocs permanents consécutifs.
...
Les séries étudiées correspondent à la base publiée en novembre 2022.
] : le niveau d'emploi aux États-Unis (série `CE16OV`, utilisée en logarithme) et les ventes au détail et services de restauration (série `RETAILx`, utilisée en logarithme, voir annexe [-@sec-autres-ex-retournement]).
Ces points de retournement ne sont pas associés à des chocs (contrairement à la crise financière de 2008 ou le COVID-) : cela permet donc de comparer les différentes méthodes lorsqu'il n'y a pas de choc.
